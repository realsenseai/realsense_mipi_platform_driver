...
- The JP6.2 VI clock floor is requested only by active `realsense,d5xx` DT sensor nodes. D4xx-only capture keeps the throughput policy; a mixed D4xx/D5xx session shares the VI clock until the last D5xx stream stops. Keep eligibility in private camera-device state and qualify any future device-family expansion separately. Apply the floor inside the locked camera clock policy; exported helpers must not recover a private allocation from a caller-supplied public struct pointer.
