...
- **`port-index` on the ds5 node's own endpoint must be unique per ds5 node** — conventionally the node's `vc-id`. It is *not* the NVCSI brick index (that lives on the VI/nvcsi endpoints); d4xx reads only `bus-width` from this endpoint. `camera_common_parse_ports()` stores it as `s_data->csi_port`, whose sole effect on the d4xx path is the debugfs directory name `camera_common_initialize()` builds as `d4xx_<csi_port + 'a'>`. Giving every ds5 node the brick index makes all of them collide on one name (`debugfs: Directory 'd4xx_c' with parent '/' already present!`, once per extra node) and only the first gets its `/sys/kernel/debug/d4xx_*/d` node.
