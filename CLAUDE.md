...
- The GMSL IMU batching register at `0x4590` applies SET synchronously: verify with one GET under the device lock. GET reports zero capability for unsupported legacy firmware so bulk V4L2 control reads still work; SET requires SUPPORTED before writing (`-EOPNOTSUPP` if absent); losing SUPPORTED after a successful preflight/write is a verification fault (`-EIO`). A failed SET after the write can leave a changed mask (including ACTIVE or I2C readback failure); callers must re-read status and perform idle cleanup rather than assuming rollback. Handle asynchronous source release in SDK close before SET; never roll back a live format from the kernel.
