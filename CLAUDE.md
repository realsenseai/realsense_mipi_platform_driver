...
- **D58x RGB calibration surfaces** — Advertise unpacked `RW16` and private CL64 `GR0P` only on a detected D58x RGB stream at 1600x1300@25. `data_type` selects the HKR DDR source while `override_dt` selects the final CSI packet DT: RW16 uses RAW16→UD30 and GR0P uses UD30→UD30. Both use opaque UD30/T_R8 carriers, preserve bytes without Host frame conversion, and leave D4x format tables and mux behavior unchanged.
