...
- **D5xx CSI metadata framing** — New firmware uses the existing 12-byte USB/UVC metadata header over CSI. Publish runtime `bytesused` from its validated `length`, keep legacy HKRM parsing for old firmware, and expose `D4XX` as the native FOURCC with `RSMD` compatibility. Never hardcode a per-stream actual metadata size; the fixed value is only queue capacity.
- **V4L2 payload-control size is ABI** — `.dims` on a `V4L2_CTRL_TYPE_U8` payload control is what userspace reads through `VIDIOC_QUERY_EXT_CTRL` to size its own buffer. librealsense sizes its GVD read that way, so a control smaller than the firmware response silently changes device identification. When a firmware response structure grows, resize the control and bound every `memcpy` by the control's `dims[0]`; a firmware-reported length must never be trusted as a copy length.
- **One overlay source per carrier** — The FG24 tunnel has exactly one DTS, `tegra234-camera-d5xx-overlay-fg24-4ch.dts`. Add virtual channels by raising `D5XX_CHANNEL_COUNT` inside it, never by adding a wrapper `.dts`. A declared channel must carry an explicit product-stream identity. `d5x_probe()` may admit or reject that identity from a device capability such as GVD; never treat the VC number itself as the SKU selector and never fall back an unsupported identity to another stream.
