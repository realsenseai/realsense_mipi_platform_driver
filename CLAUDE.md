...
## This is a public, open-source repo (MANDATORY)

Everything committed here — code, comments, commit messages, `.claude/skills/`, scripts, `CLAUDE.md` itself — is publicly visible on every pushed branch, not just the default branch. Before committing or pushing anything:

- **No internal hostnames or rig names.** Use a placeholder (`<RIG_HOST>`) or a generic description ("a D401 GMSL test rig"), never a real lab machine name.
- **No internal ticket/issue-tracker IDs** (e.g. an internal Jira key pattern) used as citations or examples — state the rule the ticket was illustrating instead of the ticket number.
- **No credentials, tokens, internal IPs, or internal-only URLs.** Per-rig secrets belong in a local, gitignored `config.ini` (see the `.gitignore` pattern already in place for skill configs) — never in a committed file.
- **No personal names, usernames, or email addresses** beyond what's already required for `Signed-off-by` trailers.
- A branch that's pushed but not yet merged is still public — this bar applies before the first push, not just before merging to `dev`.
...
./scripts/deploy_kernel.sh 6.2 <RIG_HOST> nvidia drgb_poc dev  # → /home/nvidia/drgb_poc, /boot/dev
```
...
Kernel V4L2 / media framework
    ↓
D4XX kernel driver (kernel/realsense/d4xx.c)
    ↓ I2C
...
**All max96712 D4xx overlays use the hybrid config**: camera→serializer stays **2-lane**, deserializer→Jetson runs **4-lane**. Concretely: ds5 endpoint `bus-width = <2>` and `gmsl-link/num-lanes = <2>` stay as-is; `lane_cnt = <4>` on the max96712; `bus-width = <4>` on every VI and nvcsi endpoint; `num_lanes = "4"` in every `modeN`. No d4xx.c change is involved. Any **new** max96712 D4xx overlay must be authored this way. Pure-D5xx overlays (`…-d5xx.dts`, max96717 serializer) are 4-lane end to end; max9296 and max96724 overlays are out of scope (no `lane_cnt` property).
...
- The deserializer GMSL one-shot link reset (max96712 `CTRL1` + `msleep(100)`) flushes **one** GMSL link's pixel line buffer (`link = vc_id / MAX9295_MAX_STREAMS`). It fires from exactly **two** sanctioned, d4xx-side call sites — never from `set_pipe`: (a) `ds5_hw_reset_with_recovery()`, post-HW-reset recovery; (b) `ds5_mux_s_stream()`, on GMSL link cold bring-up, via `ds5_flush_link_on_cold_bringup()` — called after `ds5_configure()` succeeds and before the `DS5_START_STOP_STREAM` write. Both gate on the same invariant: fire only when all four of the camera's `ds5_dev` streaming flags (`depth/rgb/ir/imu_streaming`) are clear, i.e. the link is idle. `__max96712_set_pipe` / `__max96712_set_multi_vc_pipe` must never flush, and a plain pipe (re)alloc must never trigger one: all of a camera's streams share one physical link, so a flush against a live link truncates a sibling's in-flight frames (PIXEL_INCOMPLETE), and a wedged link coexisting with a live stream on another link can escalate through VI error recovery into a kernel panic (NULL deref in `vi_capture_release`/`tegra_channel_error_recover`). The recovery-path flush is needed after a **camera HW reset**: the camera resets mid-frame but the serializer is not reset (keeps config + link lock), so a stale partial frame is left in the deser line buffer with nothing else to clear it. `ds5_hw_reset_with_recovery()` clears it by calling the additive per-link op `dser_interface.reset_oneshot_link(dev, vc_id)` (guarded `if (ops->reset_oneshot_link)`) after the device is ready; max96712 flushes `1 << (vc_id/MAX9295_MAX_STREAMS)`. The op is a **single unlocked `CTRL1` write** — `regmap` serializes the I2C and it does no shared-state RMW, so it needs no `priv->lock` (matching the sibling `max96712_reset_oneshot`); do **not** add a lock around it (holding `priv->lock` across its `msleep(100)` would needlessly stall a concurrent sibling `set_pipe`). max9296 wires **no** op (NULL) — its `set_pipe` re-establishes the pipe without leaving a stale line buffer, so it needs no flush (asymmetric design: max96712 flush, max9296 none).
  **Why the flush lives in the recovery path:** without it, the post-reset restart comes up on an un-flushed link, the stale frame makes the stream never complete, and VI request timeouts cascade into the fatal error-recovery path above. It cannot move to probe-time `init_settings`/`setup_link` only — camera HW reset is post-probe and re-runs neither.
  **Do not reintroduce** a serdes-side arm op (`arm_link_reset` + `pending_link_reset`) or a flush inside the max96712 `set_pipe` bodies — that machinery existed once and was removed because it fired on every pipe (re)alloc regardless of sibling state. The sanctioned cold-bring-up mechanism is d4xx-side and idle-link-gated: `ds5_dev.link_flush_pending` + `ds5_flush_link_on_cold_bringup()` — armed only while all four streaming flags are clear, consumed once, outside `set_pipe`.
  **Why the cold-bring-up flush exists:** on a shared deserializer, a camera's *first* stream after a sibling has streamed can come up wedged — FW starts, frames reach VI and are all discarded, then endless request timeouts. One per-link one-shot un-wedges it; a probe-time all-links one-shot does **not** prevent it, and restarts on an already-brought-up link need no flush.
  The coarse `reset_oneshot(0x0F)` (all-links) on the Y12I/`is_calib` path is a **separate**, ungated reset, out of scope here (all-links cross-glitches other cameras). JP5's independent `kernel/nvidia` max96712 driver carries the **same** `reset_oneshot_link` (patches `5.1.2/0007` + `5.0.2/0017`); JP7 shares JP6's via the `0006→0003` symlink. Keep all generations in sync.
...
- **Pin `model` down, never up.** An unset `model` inherits the session model, so leaving it unset is how the operator's `/model` choice reaches the fan-out. Pin explicitly only to a *cheaper* tier for mechanical stages (log collection, staged-script running, prose edits). Leave judgement stages (final review, root-cause synthesis) unset so they track the session. Never hardcode a specific higher-tier model — it silently overrides a downgrade the operator made on purpose, and the same argument applies to `effort`.
...
- **Don't re-send context you could point at.** A long facts preamble repeated to N agents is paid N times, and the agents usually re-read the same files anyway. Name the file paths and specific functions instead.
- **Right-size to the diff.** A small single-file change does not need a large adversarial fan-out. Scale the fleet to the blast radius, not to the ambition.
- **Never delegate waiting.** An agent told to run a long job (build, deploy, HW soak) and wait burns tokens and may return nothing. Use a backgrounded watcher process, or the [`soak-watch`](.claude/skills/soak-watch/SKILL.md) skill for rig soaks.
- Keep new subagents on the cheaper tier unless there is a stated reason for a higher one (the repo's `debug-agent` is the standing exception, justified for root-cause work).

## Model/effort recommendation (MANDATORY)

**Every advised action must carry a model + reasoning-effort recommendation.** This applies to anything proposed as a next step: "want me to X?", a plan step, a validation leg, a build/deploy run, a follow-up ticket task, a suggested cleanup. No advised action ships without it.
...
- **Re-recommend when scope changes.** If new evidence makes an action riskier, restate the recommendation instead of letting the old one stand.

## Status updates: timestamp + ETA (MANDATORY)

Every status/progress update to the user must open with a bracketed timestamp as its very first characters — `[HH:MM:SS, <what>, ETA ~HH:MM]` for anything still in progress, `[HH:MM:SS]` otherwise — never buried mid-sentence or omitted for chat-only status lines. This applies to any update about a build, deploy, background agent/fork, scheduled wakeup, wait on an external reply, or "still in progress" state. Recall the actual time (`date +%T` or equivalent) before writing the update; don't guess. This rule lives here because it must hold even in long, tool-dense sessions.
...
- Do **not** call `max9295_set_pipe_bpp()` during stream start — BPP writes at that point cause `CAPTURE_CHANNEL_ERROR_COLLISION` in the Tegra VI descriptor engine. The default BPP register (`0x30` = enable+16bpp) over-allocates GMSL2 link bandwidth but streaming is functionally correct.
...
- **D401 OV9782 RAW10 dword alignment — RAW8 passthrough is the fix**: The D401 VDF dword-aligns each RAW10 line: 1288px × 10/8 = 1610 B → `ALIGN(1610, 4)` = 1612 B transmitted. A pure RAW10 NVCSI capture path is not viable for this sensor:
  - The NVCSI RAW10 depacketizer requires `frame_x` to be **8-pixel aligned**. No 8-aligned value gives an expected WC of exactly 1612 B (`frame_x=1288` → 1610 B → `PIXEL_LONG_LINE`; `frame_x=1296` → 1620 B → `PIXEL_SHORT_LINE`). Non-8-aligned values cause `CAPTURE_CHANNEL_ERROR_COLLISION` regardless of WC accuracy.
...
- **VI `dt_enable`/`dt_override` semantics (HW-mapped):** with `dt_enable=1`, DT **matching must be disabled** (`match.datatype=0`, `datatype_mask=0`) or the channel never matches a frame — even an identity override (`0x2E→0x2E`) with matching kept silently times out. Line **pixel accounting always follows the wire DT** (RAW16 = 2 B/px); doubling `frame_x` to bytes gives `PIXEL_SHORT_LINE`. Overriding to a **user-defined DT (0x30) completes frames with all-zero payload** — "no processing rule" means no data, not passthrough. Overriding to RAW8 with native geometry writes mangled widened values. The **working passthrough**: `dt_override=YUV422-8 (0x1E)` + `pixfmt T_Y8_U8__Y8_V8` (enum 16) + native geometry — byte-identical payload, coexists with concurrent metadata streaming. Writer 19 (`T_U8_Y8__V8_Y8`, the YUYV row's) pair-swaps (BE); writer 17 swaps the two pixel MSBs — only 16 is identity. Shipped as `nvidia-oot/6.2/0013` (symlinked 6.0/6.1/7.0/7.1/7.2) as a **per-format-row mechanism**: `struct tegra_video_format` gained trailing `dt_override`/`pixfmt_override` fields (0 = stock; existing rows untouched), set via `TEGRA_VIDEO_FORMAT_OVERRIDE(...)`, applied row-driven in `vi5_setup_surface()` — never a fourcc if-statement in vi5_fops. No generation's `0001` format patch carries byte-swapped rows (`RW16`, plain `Y16I`) — the RGB calib surface is the **`V4L2_PIX_FMT_SGRBG16` ('GR16', upstream fourcc) override row** on the `SGRBG16_1X16` mbus, and the IR calib surface is a `Y16I` **override row** (same fourcc/32bpp geometry, now LE). Override rows also carry **transport-geometry fractions** (`frame_x_scale`/`frame_y_scale`, zero-numerator = 1/1): Y16I declares `frame_x_scale = 2/1` (32bpp V4L2 pixel on a 16bpp RAW16 wire — `frame_x` counts wire pixels, which no override can re-segment). `frame_y_scale` also scales the surface size (for flat transports like NV12-over-UD, rows = H×3/2). One structural nuance: **removing added lines from a patch can leave a context-only hunk, which `git apply` rejects as corrupt** — drop the whole no-op hunk. **Flat NV12 over UD32** (mode-agnostic): d4xx's `ds5_format` gained `override_data_type` (nonzero = the format's wire DT when it differs from the source `data_type`). **Unified contract: the FW override register always carries the wire DT** — `ds5_wire_data_type()` (`override_data_type ?: data_type`) feeds both the override register write and the serdes routing, with no camera-model branches in the value path; the D401 CSI-PT RAW8 forcing is now the SBGGR8 entries' `override_data_type` rather than inline special cases. RGB programs the register unconditionally, like depth/IR always have — every stream start (re-)asserts the format's wire DT, which displaces any stale carrier across format switches, driver reloads, and camera models alike, at the cost of one cached-suppressed register write, and the NV12 row (`RS_NV12_FLAT_1X8` mbus, a `TEGRA_VIDEO_FORMAT_GEOMETRY` row: native RAW8 match, no DT override, `frame_y_scale=3/2`) replaces the earlier side-table and `sizeimage` special cases. **The NV12 wire carrier is RAW8 (0x2A), not UD32**: the MAX96717 pixel pipe (forced BPP16 + 8-bit doubling) forwards standard 8-bit DTs but silently drops user-defined ones — UD32 gave FW-streaming/VI-silence in pixel mode; UD32 remains valid for tunnel mode only. HKR treats override-to-natural-DT as clear: a stream re-asserting its natural wire DT displaces a stale override, so format switches recover cleanly. `sizeimage` honors `frame_y_scale` generically at both channel.c sites. NV12 under the match-disabled block is not yet HW-validated (the previously validated NV12 config kept matching on). The JP7/noble `vi5_setup_surface` context differs from jammy, so `0013` has a JP7 variant: real file `nvidia-oot/7.0/0013` (7.1/7.2 symlink it); 6.x uses `nvidia-oot/6.2/0013` (the `V4L2_PIX_FMT_RW16` define in the kernel `0002`/format patches must **stay**: it predates the MIPI use, and the same patch maps `UVC_GUID_FORMAT_RW16` to it for librealsense USB devices). Duplicate-mbus rows are fine (the fmts bitmap enumerates all of them) but duplicate *fourccs* are not: `tegra_core_get_format_by_fourcc()` is first-match and not subdev-filtered (e.g. the RAW10 row already owns `SGRBG10`). Repeated aborted starts can wedge the whole GMSL link (all streams time out, no i2c errors); `rmmod d4xx && rmmod max96717 && modprobe d4xx` recovers it.
...
After every confirmed code patch, review this file against the final net diff, including any follow-up tuning edits.
...
- If the patch changed the locking, usage, or API contract of a helper or utility function, immediately update all documentation and instructions to reflect the new contract.
...
Keep comments concise and high-signal — both in-code and external (ticket/PR/status updates). Lead with the finding or the "why", in the fewest lines that stay correct; cut restated context and prose. Ticket/PR comments especially: a few skimmable lines or bullets, not walls of text.

**In-code comments: 2–3 lines maximum.** State the *why* — never narrate what the code plainly does. If one comment cannot cover it, do **not** grow the block: split it into two short comments, each placed at the spot it explains, rather than one paragraph parked above the function. Long rationale and cross-site invariants belong in the commit message, the PR description, or this file — not in a wall of comment lines.

**No comments that describe an absence or a removal.** A comment states what the code *does* (or a non-obvious *why*) — never what it no longer does, what was deleted, or what it deliberately omits. If the absence carries a real invariant worth preserving, record it once as a forward-looking rule in the commit message / PR description / this file, not as a per-site breadcrumb.

**Drafting rules for outward text (Jira/PR comments, status updates):**
- **No commit hashes, bare or as link text.** Link to the commit with a short named description of what it does (`[the sensor-timeout fix](<repo-url>/commit/<sha>)`), not the hash itself (`[abc1234](...)`) — the hash means nothing to the reader even when it's clickable.
- **No unexplained bare numbers.** A count, offset, or percentage must say what it measures inline — don't drop a number and expect the reader to infer why it matters.
- **No raw code in prose.** Describe behavior in plain English and cite `file:line`; don't paste a function/condition snippet as the explanation itself.
- **Every reference is a live link, not bare text.** A `file:line` citation, a ticket key, a doc mention — each must link to where it actually lives (GitHub, Jira, wiki, SharePoint), named by what it is, not left as unclickable text. Pin file/line links to a specific commit SHA (`.../blob/<sha>/path#L<n>`), never to a moving branch — a branch-relative link's line number silently goes stale as the file changes. If something has no public URL (e.g. a gitignored local note), link its owning ticket instead of naming an unreachable path.
- **One point per sentence.** Don't chain multiple claims through "and"/"so"/em-dash — state one claim, full stop, then the next. A reader should be able to stop after any sentence and have gained one complete fact.
- **Post Jira comments containing links via raw REST, not the MCP `jira_add_comment` tool.** That tool runs a markdown→Jira-wiki conversion that mangles underscores in URLs into `*` — this repo's own name (`realsense_mipi_platform_driver`) breaks every link to it. Write native Jira wiki markup (`[text|url]`) directly and POST it with a PAT, then verify with a raw REST GET — never an MCP read, which mangles the same way on the way back and manufactures false alarms about text that stored correctly.
