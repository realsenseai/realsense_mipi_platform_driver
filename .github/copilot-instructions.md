...
- **D5xx byte-preserving CSI surfaces**: Keep wire DT, VI parser format, and V4L2 memory ABI separate. Match the actual CSI packet DT, scope any `T_R8` parser override and byte-width `frame_x` to an exact private mbus tuple, and expose the FourCC that describes the resulting memory surface. Do not key transport behavior on a broad FourCC or add a frame copy when tuple-specific VI byte DMA can preserve the payload.
