...
if [[ "$ACTION" = "apply" ]]; then
    version_lt "$JETPACK_VERSION" "5.0" || ln -f kernel/realsense/d4xx.c "sources_${JETPACK_VERSION}/${D4XX_SRC_DST}/drivers/media/i2c/"
    if version_lt "$JETPACK_VERSION" "6.0"; then
        # device tree
        ...
        # max96712 header
        ln -f nvidia-oot/max96712.h "sources_${JETPACK_VERSION}/nvidia-oot/include/media/"
        if version_lt "$JETPACK_VERSION" "7.0"; then
            # jp6 overlay
            ln -f hardware/realsense/tegra234-camera-d4xx-overlay*.dts "sources_${JETPACK_VERSION}/hardware/nvidia/t23x/nv-public/overlay/"
            ln -f sources_${JETPACK_VERSION}/hardware/nvidia/t23x/nv-public/include/platforms/dt-bindings/tegra234-p3737-0000+p3701-0000.h \
                    sources_${JETPACK_VERSION}/$KERNEL_DIR/include/dt-bindings/
        else
            ...
            if [[ ! -f "sources_${JETPACK_VERSION}/$KERNEL_DIR/include/dt-bindings/gpio/tegra264-gpio.h" ]]; then
                ln -f "sources_${JETPACK_VERSION}/$KERNEL_DIR/3rdparty/canonical/linux-noble/include/dt-bindings/gpio/tegra264-gpio.h" \
                    "sources_${JETPACK_VERSION}/$KERNEL_DIR/include/dt-bindings/gpio/" 2>/dev/null || true
            fi
            ln -f hardware/realsense/tegra264-camera-d4xx-overlay*.dtso "sources_${JETPACK_VERSION}/$KERNEL_DIR/arch/arm64/boot/dts/nvidia/"
        fi
    fi

    # Stage all modified files after patching
    git -C "sources_${JETPACK_VERSION}/$D4XX_SRC_DST" add drivers/media/i2c/d4xx.c
    git -C "sources_${JETPACK_VERSION}/$D4XX_SRC_DST" add -u
