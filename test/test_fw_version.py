...
def test_fw_version(device):
    try:
        ...
        assert "fw version: " in std_output.decode(), "Couldn't fetch FW version"

        # Remove the 'fw version: ' string from std output
        fw_version = int(std_output.decode().replace("fw version: ", ""))
        ...
        assert "d4xx-dfu-" in dfu_device.decode(), "D4xx DFU device not found"
