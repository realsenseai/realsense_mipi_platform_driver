...
def main():
    ...
    int main(void) {
        ...
        for (unsigned mask=1;mask<=3;mask++) {
            reset(0xc0);assert(d500_set_imu_batch(&state,mask)==0);
            assert(writes==1 && wire==(0xc0|mask) && !shared.lock);
        }
        reset(0xc3);assert(d500_set_imu_batch(&state,0)==0 && wire==0xc0);
        reset(0xc0);assert(d500_set_imu_batch(&state,256)==-EINVAL && !writes);
        reset(0xc0);shared.imu_streaming=true;
        assert(d500_set_imu_batch(&state,3)==-EBUSY && !writes && !shared.lock);
        reset(0xe3);assert(d500_set_imu_batch(&state,0)==-EBUSY && !writes);
        reset(0x80);assert(d500_set_imu_batch(&state,3)==-EOPNOTSUPP && !writes);
        reset(0x80);assert(d500_set_imu_batch(&state,0)==0);
        reset(0);assert(d500_set_imu_batch(&state,3)==-EOPNOTSUPP && !writes);
        reset(0xc0);fail_write=1;assert(d500_set_imu_batch(&state,3)==-EIO);
        reset(0xc0);pending_active=true;
        assert(d500_set_imu_batch(&state,3)==-EBUSY && wire==0xe3 && writes==1 && reads==2 && !shared.lock);
        for (unsigned mask=0;mask<=3;mask++) {
            reset(0xc0);drop_capability=true;
            assert(d500_set_imu_batch(&state,mask)==-EIO);
            assert(writes==1 && reads==2 && wire==(0x40|mask) && !shared.lock);
        }
        reset(0xc0);fail_read=2;assert(d500_set_imu_batch(&state,3)==-EIO && wire==0xc3 && !shared.lock);
        reset(0xc0);delay_reads=3;assert(d500_set_imu_batch(&state,3)==-EIO && reads==2);
        reset(0xc0);delay_reads=-1;
        assert(d500_set_imu_batch(&state,3)==-EIO && reads==2 && !shared.lock);
        puts("RSDEV-13850 XU: capability, masks, busy, tunnel, I2C errors and readback tests passed");
    }
    '''
