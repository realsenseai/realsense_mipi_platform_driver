...
#define MAX96717_FRONTTOP10_ADDR	0x312
...
struct max96717 {
	...
	struct mutex lock;
	bool pixel_mode;
	/* Tracks the active VCs for the legacy MAX96712 pixel-mode path. */
	u8 active_vc_mask;
...
int max96717_setup_streaming(struct device *dev)
{
	...
	err = max96717_write_reg(dev, MAX96717_EXT11_ADDR,
				 MAX96717_TUN_MODE_EN);
	err |= max96717_write_reg(dev, MAX96717_MIPI_RX1_ADDR,
				  MAX96717_CSI_4_LANES);
	err |= max96717_write_reg(dev, MAX96717_MIPI_RX2_ADDR,
				  MAX96717_CSI_1X4_LANE_MAP1);
	err |= max96717_write_reg(dev, MAX96717_MIPI_RX3_ADDR,
				  MAX96717_CSI_1X4_LANE_MAP2);
	err |= max96717_write_reg(dev, MAX96717_VIDEO_TX0_ADDR,
				  MAX96717_VIDEO_TX0_TUN);
	err |= max96717_write_reg(dev, MAX96717_VIDEO_TX1_ADDR,
				  MAX96717_VIDEO_TX1_TUN);
	...
	/* pipes on + video init */
	err |= max96717_write_reg(dev, MAX96717_PIPE_EN_ADDR,
...
int max96717_setup_control(struct device *dev)
{
	...
	/*
	 * Existing MAX96712 designs already perform link selection and any
	 * serializer address handling in the deserializer path. Preserve that
	 * validated pixel-mode sequence and only program sensor translation.
	 */
	if (priv->pixel_mode) {
		err = max96717_write_reg(dev, MAX96717_I2C4_ADDR,
					 (g_ctx->sdev_reg << 1));
		err |= max96717_write_reg(dev, MAX96717_I2C5_ADDR,
					  (g_ctx->sdev_def << 1));
		if (!err)
...
int max96717_setup_gpio_tunneling(struct device *dev)
{
	...
	int err;
	u32 count = priv->pixel_mode ? 6 : ARRAY_SIZE(map);

	mutex_lock(&priv->lock);

	err = max96717_set_registers(dev, map, count);
	if (err)
...
/*
 * Keep the existing public symbol used by the MAX96712 pixel-mode camera
 * stack. Both names route to the same register programming.
 */
int max96717_enable_gpio_tunneling(struct device *dev)
{
	return max96717_setup_gpio_tunneling(dev);
}
...
int max96717_disable_gpio_tunneling(struct device *dev)
{
	...
	struct reg_pair pixel_map[] = {
		{MAX96717_GPIO0_A_ADDR, 0x90},
		{MAX96717_GPIO0_B_ADDR, 0x60},
		{MAX96717_GPIO0_C_ADDR, 0x00},
		{MAX96717_GPIO1_A_ADDR, 0x00},
		{MAX96717_GPIO1_B_ADDR, 0x00},
		{MAX96717_GPIO1_C_ADDR, 0x00},
	};
	int err;
	...
	if (priv->pixel_mode)
		err = max96717_set_registers(dev, pixel_map,
					     ARRAY_SIZE(pixel_map));
	else
			       ...
			       u8 data_type2, u32 vc_id)
{
	...
	if (priv->pixel_mode) {
		bpp = data_type1 == GMSL_CSI_DT_RGB_888 ? 24 : 16;

		/*
		 * Keep the existing MAX96712 pixel-mode behavior: only the
		 * serializer BPP changes when the selected stream format changes.
		 */
		return max96717_write_reg(dev, MAX96717_VIDEO_TX1_ADDR, bpp);
	}
...
{
	struct max96717 *priv = dev_get_drvdata(dev);
	u8 reset = priv->pixel_mode ? 0x48 : MAX96717_MIPI_RX0_RESET;
	u8 normal = priv->pixel_mode ? 0x40 : MAX96717_MIPI_RX0_NORMAL;
	int err;

	err = max96717_write_reg(dev, MAX96717_MIPI_RX0_ADDR, reset);
	usleep_range(2000, 2100);
	err |= max96717_write_reg(dev, MAX96717_MIPI_RX0_ADDR, normal);
...
int max96717_init_settings(struct device *dev)
{
	...
	struct reg_pair pixel_pre[] = {
		{MAX96717_PIPE_EN_ADDR, MAX96717_SOFT_RESET},
		{MAX96717_CTRL0_ADDR, MAX96717_CTRL0_LINK_A},
	};
	struct reg_pair pixel_mid[] = {
		{MAX96717_TX1_ADDR, 0x00},
		...
		{MAX96717_FRONTTOP10_ADDR, 0x04},
		{MAX96717_MIPI_RX1_ADDR, 0x30},
	};
	struct reg_pair pixel_post[] = {
		{MAX96717_MIPI_RX8_ADDR, 0x22},
		{MAX96717_PIPE_EN_ADDR, MAX96717_PIPE_EN_ALL},
	};
	/*
	...
	if (priv->pixel_mode) {
		/*
		 * Preserve the MAX96712 pixel-mode sequence already validated
		 * on dev, including the non-continuous-clock RX reset pulse.
		 */
		err |= max96717_set_registers(dev, pixel_pre,
					      ARRAY_SIZE(pixel_pre));
		msleep(100);
		err |= max96717_set_registers(dev, pixel_mid,
					      ARRAY_SIZE(pixel_mid));
		err |= max96717_mipi_rx_reset_pulse(dev);
		err |= max96717_set_registers(dev, pixel_post,
					      ARRAY_SIZE(pixel_post));
	} else {
		err |= max96717_set_registers(dev, map_init,
					     ARRAY_SIZE(map_init));
...
int max96717_stream_stop(struct device *dev, u32 vc_id)
{
	...
	mutex_lock(&priv->lock);
	priv->active_vc_mask &= (u8)~BIT(vc_id);
	if (priv->pixel_mode && !priv->active_vc_mask)
		err = max96717_mipi_rx_reset_pulse(dev);
	mutex_unlock(&priv->lock);
			   ...
			   )
{
	...
	dev_info(&client->dev, "[MAX96717]: probing GMSL2 serializer\n");
	...
	/*
	 * Pixel mode is the backwards-compatible default for all existing
	 * MAX96712 device trees. MAX96724 tunnel designs opt in explicitly.
	 */
