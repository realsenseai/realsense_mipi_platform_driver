 ...
 */
#define DS5_D58X_RGB_SATURATION		0x0030
#define DS5_D58X_RGB_SHARPNESS		0x0032
#define DS5_D58X_RGB_WHITE_BALANCE_TEMP	0x0034
#define DS5_D58X_RGB_HUE		0x0036
#define DS5_D58X_RGB_AUTO_WHITE_BALANCE	0x0038
#define DS5_D58X_RGB_POWER_LINE_FREQ	0x003A
#define DS5_D58X_RGB_AE_PRIORITY		0x003C
		...
		if (state->is_rgb)
			ret = ds5_write(state, base |
				ds5_rgb_ctrl_offset(state,
						    ...
						    DS5_D58X_RGB_WHITE_BALANCE_TEMP),
					ctrl->val);
		break;
	case V4L2_CID_HUE:
		if (state->is_rgb && ds5_is_d58x(state))
			ret = ds5_write(state, base | DS5_D58X_RGB_HUE,
					(u16)(s16)ctrl->val);
		break;
		...
		if (state->is_rgb)
			ret = ds5_write(state, base |
				ds5_rgb_ctrl_offset(state,
						    ...
						    DS5_D58X_RGB_AUTO_WHITE_BALANCE),
					ctrl->val);
		break;
...
static int ds5_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_HUE:
		if (state->is_rgb && ds5_is_d58x(state)) {
			ret = ds5_read(state, base | DS5_D58X_RGB_HUE, &reg);
			if (!ret)
				*ctrl->p_new.p_s32 = (s16)reg;
		}
		break;
