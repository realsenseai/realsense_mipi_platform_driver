...
if [ "${JETPACK_VERSION}" = "5.0.2" ]; then
    ...
    if [ -z "${D4XX_MODULE}" ]; then
        echo "Error: Extracted installation requires the unified d4xx.ko"
        echo "Existing target modules were not modified."
        exit 1
    fi
    # JP7 build output intentionally omits the NVIDIA display stack. Overlay
    # the generated tree so the BSP-matched display modules remain installed.
